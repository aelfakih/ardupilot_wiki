...
.. note:: The camera mount function and gimbal drivers/features are NOT normally included on standard firmware for smaller flash (F4) boards. Use the `Custom Firmware Build Server <https://custom.ardupilot.org/>`__ to create firmware that includes it.
